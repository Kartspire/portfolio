...
export async function getClient(clientId) {
  const responce = await fetch(`http://localhost:3000/api/clients/${clientId}`);
  return await responce.json();
}
...
export async function patchClient(client, clientId) {
  const responce = await fetch(`http://localhost:3000/api/clients/${clientId}`, {
    method: 'PATCH',
    headers: { 'Content-type': 'application/json' },
    body: JSON.stringify({
...
export function renderClient(client) {
  const clientElement = el('li', { class: 'client list-group-item row d-flex' }, [
    el('div', { class: 'col-2', id: 'client-id' }, client.id),
    el('div', { class: 'col-2' }, `${client.surname} ${client.name} ${client.lastName}`),
    el(
      'div',
      { class: 'col-2' },
      `${reformTime(new Date(client.createdAt)).reformedDate} ${reformTime(new Date(client.createdAt)).reformedTime}`
    ),
    el(
      'div',
      { class: 'col-2' },
      `${reformTime(new Date(client.updatedAt)).reformedDate} ${reformTime(new Date(client.updatedAt)).reformedTime}`
    ),
    renderContactIcons(client),
    el('div', { class: 'col-2 d-flex justify-content-between', 'data-client-id': `${client.id}` }, [
      el('a', { 'data-bs-toggle': 'modal', 'data-bs-target': '#patch-client-modal' }, 'изменить'),
      el(
        'a',
        {
